...
name: "Order",
  ...
  methods:{
    ...
    async addOrders(){
      console.log(this.selectBooks.length);
      console.log(this.selectBooks);
      let data =[]
      for(let i =0;i<this.selectBooks.length;i++){
        data[i] = {}
        data[i].tel = this.orderForm.tel
        data[i].address = this.orderForm.street_name
        data[i].name = this.orderForm.name
        data[i].email = this.user.u_email
        data[i].cost = this.selectBooks[i].books_secprice * this.selectBooks[i].books_sums
        data[i].books_id = this.selectBooks[i].books_id
        data[i].books_name = this.selectBooks[i].books_name
        data[i].books_price = this.selectBooks[i].books_secprice
        data[i].books_sum = this.selectBooks[i].books_sums
        data[i].cart_id = this.selectBooks[i].cart_id
        data[i].state = this.$store.getters.orderState
        console.log(this.$store.getters.orderState);
        await addOrder(data[i]).then(res =>{
             console.log(res);
        })
      }
